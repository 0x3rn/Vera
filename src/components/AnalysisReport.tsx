...
function ExpandableDrawer({ 
  ...
  return (
    <div className={`border rounded-xl bg-white dark:bg-card overflow-hidden transition-all duration-300 ${isOpen ? 'border-primary/50 ring-1 ring-primary/20 shadow-lg' : 'border-border'}`}>
      <button 
        onClick={onToggle}
        className="w-full p-6 flex items-center justify-between bg-white dark:bg-card hover:bg-slate-50 dark:hover:bg-muted/50 transition-colors"
      >
...
export default function AnalysisReport({ analysis }: AnalysisReportProps) {
  ...
  let verdictText = "";
  if (score >= 80) {
    verdictClass = "bg-rose-600 border-rose-700 text-white dark:bg-rose-950/40 dark:border-rose-900/50 dark:text-rose-400";
    verdictText = "CRITICAL WARNING: DO NOT SIGN";
  } else if (score >= 45) {
    verdictClass = "bg-amber-500 border-amber-600 text-white dark:bg-amber-950/40 dark:border-amber-900/50 dark:text-amber-400";
    verdictText = "WARNING: PROCEED WITH EXTREME CAUTION";
  } else {
    verdictClass = "bg-emerald-600 border-emerald-700 text-white dark:bg-emerald-950/40 dark:border-emerald-900/50 dark:text-emerald-400";
  ...
  return (
    <div className="w-full space-y-10 animate-in fade-in zoom-in duration-500">
      ...
      <div className="p-8 rounded-2xl bg-white dark:bg-card border border-border">
        <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-500 dark:text-muted-foreground mb-4">Executive Summary</h3>
        <p className="text-zinc-800 dark:text-muted-foreground leading-relaxed text-lg font-medium">{analysis.summary}</p>
      </div>
      ...
      {analysis.dealBreakers && analysis.dealBreakers.length > 0 && (
        <div className="p-8 rounded-2xl bg-white dark:bg-rose-950/10 border border-rose-300 dark:border-rose-500/30">
          <h3 className="text-sm font-bold uppercase tracking-widest text-rose-600 dark:text-rose-400 mb-6 flex items-center gap-2">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <ul className="space-y-3">
            {analysis.dealBreakers.map((breaker, i) => (
              <li key={i} className="flex items-start gap-3 text-rose-600 dark:text-rose-200 font-medium">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        {analysis.financialExposure && (
          <div className="p-8 rounded-2xl bg-white dark:bg-card border border-border flex flex-col">
            ...
            <div className="mt-4 p-5 bg-white dark:bg-rose-500/10 rounded-xl border border-rose-300 dark:border-rose-500/20">
              <p className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-widest mb-1">Total Estimated Exposure</p>
              <p className="text-lg font-bold text-rose-700 dark:text-rose-200">{analysis.financialExposure.totalEstimatedExposure}</p>
            </div>
          </div>
        )}
        ...
        {analysis.worstCaseScenario && (
          <div className="p-8 rounded-2xl bg-white dark:bg-card border border-border flex flex-col">
            ...
            <div className="flex-grow flex items-center">
              <p className="text-zinc-800 dark:text-muted-foreground font-medium leading-relaxed text-sm italic border-l-2 border-rose-500/50 pl-4 py-2 bg-slate-50 dark:bg-muted/30 rounded-r-xl">
                {analysis.worstCaseScenario}
              </p>
            </div>
          </div>
      ...
      {analysis.negotiationChecklist && analysis.negotiationChecklist.length > 0 && (
        <div className="p-8 rounded-2xl bg-white dark:bg-indigo-950/10 border border-indigo-300 dark:border-indigo-500/30">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <button 
              ...
              className="px-4 py-2 text-xs font-bold uppercase tracking-wider bg-indigo-100 hover:bg-indigo-200 dark:bg-indigo-500/10 dark:hover:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 rounded-lg border border-indigo-300 dark:border-indigo-500/30 transition-colors flex items-center gap-2"
            >
              {copied ? (
                <>
          ...
          <div className="space-y-3 p-5 bg-white dark:bg-background rounded-xl border border-border shadow-sm">
            {analysis.negotiationChecklist.slice(0, 5).map((item, i) => (
              <div key={i} className="flex items-start gap-3">
                ...
                <p className="text-zinc-800 dark:text-foreground text-sm md:text-base font-semibold">{item}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {analysis.positiveFindings && analysis.positiveFindings.length > 0 && (
        <div className="p-8 rounded-2xl bg-white dark:bg-emerald-950/10 border border-emerald-300 dark:border-emerald-500/30">
          ...
          <ul className="space-y-3">
            {analysis.positiveFindings.map((finding, i) => {
              ...
              return (
                <li key={i} className="flex items-start gap-3 text-emerald-600 dark:text-emerald-200 font-medium">
          ...
          <div className="space-y-6 mt-4">
            {sortedFlags.length === 0 ? (
              ...
              sortedFlags.map((flag: any, idx: number) => {
                ...
                const borderClass = isCritical ? "border-rose-300 dark:border-rose-600/50" : isHigh ? "border-red-300 dark:border-red-500/30" : isMedium ? "border-amber-300 dark:border-amber-500/30" : "border-blue-300 dark:border-blue-500/30";
                const bgClass = isCritical ? "bg-white dark:bg-rose-950/20" : "bg-white dark:bg-card";
                  ...
                  <div key={idx} className={`p-6 md:p-8 rounded-2xl border ${borderClass} ${bgClass} shadow-sm`}>
                    <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                      <div>
                        ...
                        <h4 className={`text-xl font-bold ${isCritical ? 'text-rose-700 dark:text-rose-100' : 'text-foreground'}`}>{flag.title}</h4>
                      </div>
                      <div className="flex flex-wrap gap-2 shrink-0">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${
                            isCritical ? "bg-rose-100 dark:bg-rose-600/20 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-600/40" : isHigh ? "bg-red-100 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-200 dark:border-red-500/30" : isMedium ? "bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/30" : "bg-blue-100 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-500/30"
                          }`}
                        >
                        ...
                        {flag.enforcementLikelihood && (
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border ${
                            flag.enforcementLikelihood === 'High' ? "bg-rose-100 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-200 dark:border-rose-500/30" : 
                            flag.enforcementLikelihood === 'Medium' ? "bg-amber-100 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/30" : 
                    ...
                    <p className="text-zinc-800 dark:text-muted-foreground mb-6 leading-relaxed text-base font-medium">{flag.plainEnglishExplanation}</p>
                    
                    <div className="bg-white dark:bg-emerald-500/10 border border-emerald-300 dark:border-emerald-500/20 p-5 rounded-xl">
                      <p className="text-sm text-emerald-700 dark:text-emerald-300 font-medium">
                        <span className="font-bold text-emerald-600 dark:text-emerald-400 block mb-1">Recommendation:</span> {flag.suggestedFix}
                      </p>
                    </div>
                  </div>
                );
              ...
              {(!analysis.clauseConflicts || analysis.clauseConflicts.length === 0) ? (
                ...
                <div className="space-y-4">
                  {analysis.clauseConflicts.map((conflict, i) => {
                    ...
                    return (
                      <div key={i} className="p-5 bg-white dark:bg-background rounded-xl border border-border">
                        ...
                        <p className="text-sm text-amber-800 dark:text-amber-200/80 leading-relaxed"><span className="font-bold text-amber-600 dark:text-amber-400">Effect: </span>{conflict.explanation}</p>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="space-y-6">
                  {analysis.riskChains.map((chain, i) => (
                    <div key={i} className="p-5 bg-white dark:bg-background rounded-xl border border-border">
                      ...
                      <p className="text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed"><span className="text-cyan-700 dark:text-cyan-500 font-bold mr-2">Loophole Effect:</span>{chain.effect}</p>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="p-5 rounded-xl bg-white dark:bg-card border border-border mt-8">
