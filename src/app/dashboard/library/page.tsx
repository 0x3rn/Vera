  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {categoriesArray.length === 0 ? (
        <div className="bg-card border border-border rounded-2xl p-8 sm:p-12 text-center max-w-2xl mx-auto">
          <div className="w-12 h-12 mx-auto mb-4 rounded-xl bg-primary/10 flex items-center justify-center">
            <svg className="w-6 h-6 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25" />
            </svg>
          </div>
          <p className="text-muted-foreground mb-4">You have no risks in your library yet. Scan a contract to start building your knowledge base.</p>
            ...
            <div key={category} className="bg-card border border-border rounded-2xl p-6">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">Appeared in {data.count} clauses</p>
                </div>
              ...
              <div className="space-y-4">
                <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Examples found in your contracts</p>
                {data.examples.map((example, idx) => (
                  <Link key={idx} href={`/dashboard/results/${example.scanId}`} className="block group">
                    <div className="bg-muted border border-border rounded-xl p-4 hover:border-primary/30 transition-colors">
                      <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors mb-2 leading-relaxed">
                        "{example.title}"
                      </p>
                      <p className="text-xs text-muted-foreground">— {example.documentName}</p>
                    </div>
                  </Link>
                ))}
              </div>
